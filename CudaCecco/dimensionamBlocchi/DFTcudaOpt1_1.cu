...
*/
    ...
    dim3 blockDFT((img.width + threadsDFT.x - 1) / threadsDFT.x, (img.height + threadsDFT.y - 1) / threadsDFT.y);

    dim3 threadsFIL(16, 16);
    dim3 blockFIL((img.width + threadsFIL.x - 1) / threadsFIL.x, (img.height + threadsFIL.y - 1) / threadsFIL.y);
    ...
    dft_opt1<<<blockDFT, threadsDFT>>>(d_in, d_dft, img.width, img.height);
    ...
    filtro<<<blockFIL, threadsFIL>>>(d_dft, img.width, img.height, raggioFiltro);
    CHECK(cudaDeviceSynchronize());
    ...
    idft_opt1<<<blockDFT, threadsDFT>>>(d_dft, d_out, img.width, img.height);
    CHECK(cudaDeviceSynchronize());
