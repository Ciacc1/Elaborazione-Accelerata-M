...
__global__ void idft_opt(MyComplex *in, unsigned char *out, int width, int height) {
    ...
    for (int v = 0; v < height; v++) {
        ...
        // CONTROLLO SAFE PATH GLOBALE (Multiplo di 16)
        if (width % 16 == 0) {

            // Il ciclo esterno salta di 16 in 16
            for (int u = 0; u < width; u += 16) {

                // Il ciclo interno elabora i 16 elementi locali.
                // Essendo il limite "16" una costante, nvcc fa l'unroll 
                #pragma unroll 16
                for (int i = 0; i < 16; i++) {

                    int current_u = u + i; // L'indice reale del pixel

                    float cosU, sinU;
                    sincosf(angleX * current_u, &sinU, &cosU);

                    float c = __fmaf_rn(cosU, cosV, -sinU * sinV);
                    float s = __fmaf_rn(sinU, cosV,  cosU * sinV);

                    MyComplex coeff = in[v * width + current_u];
